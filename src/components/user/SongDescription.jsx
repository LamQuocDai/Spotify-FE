...
const SongDescription = () => {
  const { currentSong, audio, setIsPlaying, isPlaying, currentTime } =
    useAudio();
      ...
      if (document.hidden) {
        ...
        if (isPlaying) {
          videoElement
            .play()
            .catch((error) =>
              console.error("Play error after tab change:", error)
            );
        }
      }
    };
    ...
    if (currentSong.url_video.endsWith(".m3u8") && Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        if (isPlaying && !document.hidden) {
          videoElement
            .play()
            .catch((error) => console.error("HLS play error:", error));
        }
      });
    } else {
      ...
      {currentSong && (
        <>
          <video
            ...
            muted
            preload="auto"
            className="w-full h-auto rounded-lg mt-4 hide-native-controls"
          />
